...
class Analyzer(object):
    ...
    def simulate(self, n=-1):
        ...
        if type(n) == str:
            ...
            n = len(self.trader.price[
                        pair]) - n  # translate the input n (lookback window) into the index n used in this context
            start_time = self.trader.price[pair][n][0]
        ...
        for key in self.trader.price[pair][n:]:
            ...
            for pair in sorted(advice, key=lambda key: advice[key], reverse=True):
                ...
                amountBuy = advice[pair] * self.trader.price[pair][elem[pair]][2] * (
                1 - self.account.asset_pair[pair]['fees'][0][1] / 100)
            ...
            for pair in sorted(advice, key=lambda key: advice[key], reverse=True):
                ...
                sellFX = pair[4:]
                elem[pair] = hf.get_closest_elem(self.trader.price[pair], key[0], elem[pair])
                sellAmount = advice[pair] * self.trader.price[pair][elem[pair]][1] * (
                1 - self.account.asset_pair[pair]['fees'][0][1] / 100)
            ...
            if not self.optimize:
                ...
                print
                str(key[0]) + " " + str(i) + ", Equivalent in " + self.reference_curr + ": " + str(round(eq_bal, 2)) + \
                ",\n\t Compared to market (" + str(round(s_eq_bal, 2)) + "): " + str(
                    round((eq_bal / s_eq_bal - 1) * 100, 2)) + \
                "%,\n\t Compared to start (" + str(round(start_bal, 2)) + "): " + str(
                    round((eq_bal / start_bal - 1) * 100, 2)) + "%." + \
                "\nRelative balances[%]: " + str(sorted(rel_bal.items(), key=lambda x: x[1], reverse=True))

        print
        "Start balance: " + str(start_bal)
        print
        "Market adjusted end balance: " + str(end_bal)
        print
        "Return: " + str(eq_bal / start_bal * 100 - 100) + "%"
        print
        "-----------------------\nSingle Positions:"
        for bal in balance:
            print
            str(bal) + ": " + str(balance[bal])
        return eq_bal
    ...
    def gradient(self, vec=np.empty([0])):
        ...
        for i in range(len(g)):
            ...
            g[i] = (f_x_eps - f_x) / eps
        print
        "Current gradient: " + str(g)

        print
        "Starting adaptive stepsize algorithm"
        if np.linalg.norm(g) > 0.5:
            g = g / np.linalg.norm(g) / 1000  # reduce size of g

        if (self.stepsize(g, f_x, 1.1,
                          sim_length) == g).all():  # i.e. no changes, as the gradient would lead to not allowed constant values
            print
            "Optimal Constants Found after " + str(self.iter_count) + " Iterations: " + str(self.trader.constant)
        ...
        if self.iter_count < 100:
            ...
            print
            str(self.iter_count) + " Iterations: " + str(self.trader.constant)
            # self.trader.write_new_trader()
            if np.linalg.norm(g) < eps:  # avoid too small gradiants
                print
                "Stopping Optimization because of too small gradient"
            else:
                self.gradient()
        else:
            print
            "Optimal Constants After 99 Iterations: " + str(self.trader.constant)

    def stepsize(self, g, f_x, size, sim_length=-1):

        for i in range(len(g)):
            if self.trader.constant[self.trader.constant["float"][i]] + float(g[i]) > 0:
                self.trader.constant[self.trader.constant["float"][i]] = self.trader.constant[
                                                                             self.trader.constant["float"][i]] + float(
                    g[i])
            else:
                return g
        ...
        if f_x_g > f_x:
            print
            "Next adaptive stepsize: " + str(g * size).replace("\n", "")
            print
            "Last optimal Equivalent Balance: " + str(f_x_g)
