...
def get_closest_elem(price,time,elem = 0):
    if not elem or abs(time-price[elem][0])>dt.timedelta(1):# if no initial elem is given or the delta is too big, search with numpy
        return np.argmin(np.abs(np.matrix(price)[:,0]-time))
    elif elem+1 < len(price) and time > price[elem][0] and (abs(time - price[elem+1][0]) < abs(time - price[elem][0])): #Step forward
        return get_closest_elem(price,time,elem+1)
    elif elem-1 >= 0 and time < price[elem][0] and (abs(time - price[elem-1][0]) < abs(time - price[elem][0])): #Step backward
        return get_closest_elem(price,time,elem-1)
    return elem
