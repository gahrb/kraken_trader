...
def populateDB(k):
    ...
    for pair in trade_pairs:
        ...
        if 'result' in query.keys():
            updateDB(query['result'][pair],pair)
        else:
...
def get_asset_pairs(k):
    ...
    for pair in pairs['result']:
        pair = pair.replace(".d","")
        if not(pair in trade_pairs or pair.find("CAD")>=0 or  pair.find("USD")>=0 or pair.find("GBP")>=0 or pair.find("JPY")>=0):
            print pair
...
def updateDB(res,pair):
    ...
    try:
        ...
        cur.execute("INSERT INTO "+pair+" VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",(datetime.now(),
